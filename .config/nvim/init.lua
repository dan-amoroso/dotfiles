...
do
  ...
  -- Allow loading of project-specific .nvim.lua / .exrc files
  vim.o.exrc = true
  ...
  -- Muted gray, italic virtual-text diagnostics — re-applied on colorscheme load
  --  so the override survives `:colorscheme` swaps.
  vim.api.nvim_create_autocmd('ColorScheme', {
    group = vim.api.nvim_create_augroup('custom-diagnostic-colors', { clear = true }),
    callback = function()
...
do
  ...
  require('which-key').setup {
    ...
    spec = {
      ...
      { '<leader>l', group = '[L]SP' },
      { '<leader>p', group = '[P]aredit', mode = { 'n', 'v' } },
      { '<leader>w', group = '[W]orkspace' },
      { '<leader>t', group = '[T]oggle' },
      { '<leader>h', group = 'Git [H]unk', mode = { 'n', 'v' } }, -- Enable gitsigns recommended keymaps first
      { '<leader>e', group = '[E]valuate', mode = { 'n', 'v' } },
    },
  }
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      -- All LSP actions are grouped under <leader>l
      ...
      map('<leader>ls', vim.lsp.buf.document_symbol, '[S]ymbols')
      map('<leader>lS', vim.lsp.buf.workspace_symbol, '[S]earch Workspace Symbols')
      map('<leader>lh', vim.lsp.buf.hover, '[H]over Documentation')
      map('<leader>la', vim.lsp.buf.code_action, 'Code [A]ctions', { 'n', 'x' })
      map('<leader>lR', vim.lsp.buf.rename, '[R]ename')
      map('<leader>lf', function() vim.lsp.buf.format { async = true } end, '[F]ormat')
      map('<leader>lq', vim.diagnostic.setloclist, '[Q]uickfix List')
      map('<leader>lt', '<cmd>Trouble diagnostics toggle<cr>', '[T]rouble')
      map('<leader>le', vim.diagnostic.open_float, 'Show [E]rror')

      -- Diagnostic navigation (using non-deprecated vim.diagnostic.jump)
...
-- ============================================================
-- SECTION 10: EXTRA UTILITIES
-- trouble.nvim, project autocmds
-- ============================================================
do
  -- Better diagnostic management — used by <leader>lt
...
do
  require('custom.plugins.visual_surround').setup()
end
